class WordCloudData {
  ...
  extractWordsFromInput(str) {
    ...
    const ellipsesRemoved = spacesRemoved.reduce(
      (accum, current) => accum.concat(current.split('...')),
      []
    );
    const invalidPunctuationRemoved = ellipsesRemoved.map((word) =>
      this.removeInvalidPunctuation(word)
    );
    const filteredWords = invalidPunctuationRemoved.filter(
      (word) => !this.isOnlyPunctuation(word)
    );
    const desensitizedWords = filteredWords.map((word) =>
      word
        .split('')
